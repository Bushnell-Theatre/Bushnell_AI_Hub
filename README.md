...
| Access/SSO wording (Claude & ChatGPT via Bushnell SSO; Copilot via Bushnell Microsoft 365 account — never mix these up) | Both files, wherever Claude/ChatGPT/Copilot access is mentioned (tool notes, FAQ, Myths, Rules). This wording currently only describes Bushnell's own SSO/workspace setup — extending it to Warner or HSO requires a confirmed access decision, not an assumption; see the flagged locations noted in commit history |
